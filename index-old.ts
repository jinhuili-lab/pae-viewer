...
`;
  ...
  private _colorRange: any = entityColors;
  ...
  get entities(): E[] | undefined {
    return this._entities;
  }

  set entities(value: E[] | undefined) {
    this._entities = value;
  }

  get entityColorScale(): EntityColorScale<E> {
    return this._entityColorScale;
  }

  set entityColorScale(value: EntityColorScale<E>) {
    this._entityColorScale = value;
  }

  get paeColorScale(): PaeColorScale {
    return this._paeColorScale;
  }

  set paeColorScale(value: PaeColorScale) {
    this._paeColorScale = value;
  }

  get pae(): number[][] | undefined {
    return this._pae;
  }

  set pae(value: number[][] | undefined) {
    this._pae = value;
  }


  private _pae: number[][] | undefined;
  private _paeColorScale: PaeColorScale
    = value => Array(3).fill(value * 255) as any as RgbColor;
  private _entityColorScale: EntityColorScale<E>
    = (_, i) => entityColors[i % entityColors.length];
  private _entities: E[] | undefined;

  constructor(
    root: HTMLElement,
    style = {},
    paeColorScale?: PaeColorScale,
    entityColorScale?: EntityColorScale<E>
  ) {
    ...
    if (paeColorScale) {
      this._paeColorScale = paeColorScale;
    }

    if (entityColorScale) {
      this._entityColorScale = entityColorScale;
    }

    const axesGroup = this._element.querySelector(".pv-axes") as SVGGElement;

    for (const line of axesGroup.querySelectorAll("line")) {
  ...
  set(pae: number[][], entities?: E[]) {
    ...
    this._dim = pae.length;

    this._createImage(pae).then(url => {
  ...
  load(complex: any, colorMapping = null) {
    ...
    if (Object.hasOwn(complex, "pae") && Object.hasOwn(complex, "maxPae")) {
      this._createImage(complex.pae).then((url) =>
        this._setPaeImage(url)
    ...
    fetch(imageUrl)
      ...
      .then((image) => {
        ...
        reader.onloadend = () => {
          ...
          return new Blob([outerSvg.outerHTML], {
  ...
  private _createImage(pae: number[][], colorScale: (value: number) => RgbColor):
    Promise<Blob> {
    const dim = pae.length;
    const rgbaValues
      :
      number[] = new Array(dim ** 2 * 4);
...
.
  ...
  subunit;
.
  color = colorMapping[subunit.uniprot];
}
} else
if (colorRange !== null) {
...
private
_updateRegionSelectionStatus(region
:
any;
)
{
